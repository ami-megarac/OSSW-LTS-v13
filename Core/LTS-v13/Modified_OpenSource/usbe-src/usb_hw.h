...
/* USB Workqueue structure used to dump Ethernet over USB packets
 * if /proc/sys/ractrends/usbe/DebugLevel is set appropriately */
typedef struct workqueue_struct USB_ETH_DATA_WORKQUEUE;
typedef struct
{
	struct work_struct 	EthWork;
	uint8 			Message[256];
	uint8			Ep;
	uint8			DevNo;
	uint8			Buffer[700];
...
extern USB_ETH_DATA_WORKQUEUE *UsbEthWorkQueue;
