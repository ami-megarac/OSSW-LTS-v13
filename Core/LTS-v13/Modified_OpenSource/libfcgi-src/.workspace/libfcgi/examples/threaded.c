...
static void *doit(void *a)
{
    ...
    for (;;)
    {
		...
		{
			printf("Acquire pthread_mutex_lock () failed.\n");
			return -1;			
		}
        rc = FCGX_Accept_r(&request);
        if(0 != pthread_mutex_unlock(&accept_mutex))
		{
			printf("Acquire pthread_mutex_lock () failed.\n");
			return -1;
		}
        ...
        if(0 != pthread_mutex_lock(&counts_mutex))
		{
			printf("Acquire pthread_mutex_lock () failed.\n");
			return -1;			
		}
        ...
        if(0 != pthread_mutex_unlock(&counts_mutex))
		{
			printf("Acquire pthread_mutex_lock () failed.\n");
			return -1;
		}
