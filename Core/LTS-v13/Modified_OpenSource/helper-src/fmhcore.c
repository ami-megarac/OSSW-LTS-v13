...
CreateFMH(FMH *fmh,unsigned long AllocatedSize, MODULE_INFO *mod)
{
	...
	/* Fill the FMH Fields */		
	ret = snprintf(fmh->FMH_Signature,sizeof(fmh->FMH_Signature),"%s",FMH_SIGNATURE);
	if(ret < 0 || ret >= (signed)sizeof(fmh->FMH_Signature))
	{
...
CreateAlternateFMH(ALT_FMH *altfmh,unsigned long FMH_Offset) 
{
	...
	/* Fill the FMH Fields */		
	ret = snprintf(altfmh->FMH_Signature,sizeof(altfmh->FMH_Signature),"%s",FMH_SIGNATURE);
	if(ret < 0 || ret >= (signed)sizeof(altfmh->FMH_Signature))
	{
		printk(KERN_ERR "Buffer Overflow\n");
		return;
	}
