...
/* Workqueue used for dumping Ethernet over USB packet(s) */
USB_ETH_DATA_WORKQUEUE *UsbEthWorkQueue = NULL;
EXPORT_SYMBOL_GPL(UsbEthWorkQueue);
...
int register_usb_device (usb_device_driver* usb_device_driver, USB_DEV* UsbDev)
{
	...
	/* Try creating workqueue for Ethernet over USB device alone
	 * as we might need to dump the packet(s) for debugging purpose */
	if(DevInfo[DevNo].UsbDevType == CREATE_ETH_DESCRIPTOR)
	{
		UsbEthWorkQueue = create_singlethread_workqueue("usbe_eth_queue");
		if(UsbEthWorkQueue == NULL)
			printk("Error in creating the workqueue (Ethernet over USB data dump will not work)\n");
	}
...
int unregister_usb_device (usb_device_driver *usb_device_driver)
{
	...
	/* Flush and destroy the workqueue used for dumping Ethernet over USB packet(s) */
	if ( (UsbEthWorkQueue != NULL) && (DevInfo[DevNo].UsbDevType == CREATE_ETH_DESCRIPTOR) )
	{
		flush_workqueue(UsbEthWorkQueue);
		destroy_workqueue(UsbEthWorkQueue);
	}
