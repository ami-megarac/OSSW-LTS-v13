...
{
    if(DescTable [DevNo].NextCfgId <= 0)
    {
        TCRIT(" ERROR: GetNumConfigurations for Device 0x%x and NextCfgId %d ",DevNo, DescTable [DevNo].NextCfgId);
        return 0;
    }
	//return an unsigned value
    return (uint8) (DescTable [DevNo].NextCfgId - 1);
}
...
GetDeviceQualifierDesc(uint8 DevNo)
{
	...
	DescTable [DevNo].DevQualifier.bcdUSBL 			= 0x00;
	DescTable [DevNo].DevQualifier.bcdUSBH 			= 0x02;		/* USB 2.0 */
#else
								...
								char *ManufStr, char*ProdStr, int IsThisLunDev)
{
	...
	if(NULL != DevInfo[DevNo].UsbDevice.UsbHwGetdevicetypeEps)
	{
		if(DevInfo[DevNo].UsbDevice.UsbHwGetdevicetypeEps (DevNo, (uint8*)&DevEpInfo[DevNo].ep_config[0], &DevEpInfo[DevNo].num_eps))
		{
			TCRIT ("Error in getting EPs information from usb HW\n");
			return -1;
		}
	}
...
/* Add OS string descriptor */
int 
AddOsStringDesc(uint8 DevNo,uint8 VendorCode,char *VendSigStr)
{

    USB_OS_STRING_DESC *OsStrDesc;
    uint8 i;

    OsStrDesc = (USB_OS_STRING_DESC *)OsString;

    if (!VendSigStr) 
         return 1;
 
    OsStrDesc->bLength 			= sizeof(USB_OS_STRING_DESC);
    OsStrDesc->bDescriptorType 	= STRING;
    OsStrDesc->bMS_VendorCode 	= VendorCode;
    OsStrDesc->bPad 	= 0x00;
 
    i=0;
    while(*VendSigStr)
	{
		OsStrDesc->qwSignature[i] 	= *VendSigStr;
		OsStrDesc->qwSignature[i+1] = 0;
		i+=2;
		VendSigStr++;
	}

    DescTable[DevNo].StringTable[0xEE] = OsString;
	DescTable[DevNo].StringLen[0xEE] = OsStrDesc->bLength;

    return 0;

}

/* Add Extended properties descriptor */
int 
AddExtPropertiesDesc(char *CustomPropertyStr)
{

    USB_EXTENDED_PROPERTY_DESC *ExtPropDesc;
    uint8 i;

    ExtPropDesc = (USB_EXTENDED_PROPERTY_DESC *)ExtProp;

 
    ExtPropDesc->dwLength               = 0x4C; // size of descriptor(=76)
    ExtPropDesc->bcdVersion             = 0x0100;
    ExtPropDesc->wIndex                 = 0x05;
    ExtPropDesc->wCount                 = 0x01;
    ExtPropDesc->dwSize                 = 0x42;
    ExtPropDesc->dwPropertyDataType     = 0x04; // REG_DWORD_LITTLE_ENDIAN
    ExtPropDesc->wPropertyNameLength    = 0x30;
    ExtPropDesc->dwPropertyDataLength   = 0x04;
    ExtPropDesc->bPropertyData          = 0x01;

    i=0;
    while(*CustomPropertyStr)
	{
		ExtPropDesc->bPropertyName[i] 	= *CustomPropertyStr;
		ExtPropDesc->bPropertyName[i+1] = 0;
		i+=2;
		CustomPropertyStr++;
	}

    ExtPropDesc->bPropertyName[i]       = 0;
    ExtPropDesc->bPropertyName[i+1]     = 0;
