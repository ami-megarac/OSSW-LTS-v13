...
/* Macros used in Ethernet over USB data dump */
#define USBE_ETH_RECEIVE_MESSAGE "USBRxHandler(): Received following data via Ethernet over USB"
#define USBE_ETH_SEND_EOTF_MESSAGE "HandleTxData(): End of Transmission - Full packets sending following data via Ethernet over USB"
#define USBE_ETH_SEND_FULP_MESSAGE "HandleTxData(): Full packets sending following data via Ethernet over USB"
#define USBE_ETH_SEND_EOTP_MESSAGE "HandleTxData(): End of Transmission Partial packets sending following data via Ethernet over USB"
#define USBE_ETH_SEND_ZERO_MESSAGE "HandleTxData(): End of Transfer zero length packets sending following data via Ethernet over USB"
...
UsbRxHandler(uint8 DevNo,uint8 ep)
{
	...
	/* Dump Ethernet over USB packet(s), if the relevant debug level is enabled */
	if( TDBG_IS_DBGFLAG_SET(usbe, DEBUG_DUMP_ETH_DATA) && (DevInfo[DevNo].UsbDevType == CREATE_ETH_DESCRIPTOR))
	{
		USB_ETH_DATA_WORK *Req  = NULL;
		uint8 EpIndex = 0;

		/* Post the packet(s) to the workqueue */
		if(UsbEthWorkQueue)
		{
			EpIndex = GetEndPointIndex (DevNo, ep, DIR_OUT);
			Req = (USB_ETH_DATA_WORK *)kmalloc(sizeof(USB_ETH_DATA_WORK), GFP_ATOMIC);
			if( (Req != NULL) && (EpIndex != 0xFF) )
			{
				if(sizeof(Req->Message) >= sizeof(USBE_ETH_RECEIVE_MESSAGE))
				{
					memcpy(Req->Message, USBE_ETH_RECEIVE_MESSAGE, sizeof(USBE_ETH_RECEIVE_MESSAGE));
				}
				Req->Ep = ep;
				Req->DevNo = DevNo;
				if(sizeof(Req->Buffer) >= DevInfo[DevNo].EndPoint[EpIndex].RxDataLen)
				{
					memcpy(Req->Buffer, DevInfo[DevNo].EndPoint[EpIndex].RxData, DevInfo[DevNo].EndPoint[EpIndex].RxDataLen);
				}
				Req->Len = DevInfo[DevNo].EndPoint[EpIndex].RxDataLen;
				Req->Jiffies = jiffies;
				INIT_WORK(&Req->EthWork, UsbCoreDumpEthData);
				queue_work(UsbEthWorkQueue, &Req->EthWork);
			}
			else
			{
				if(Req == NULL)
					printk("Couldn't allocate enough memory to dump Ethernet over USB packet(s)");
				if(EpIndex == 0xFF)
		...
		if (EpInfo->TxData == NULL)
		{
			...
			if( TDBG_IS_DBGFLAG_SET(usbe, DEBUG_DUMP_ETH_DATA) && (DevInfo[DevNo].UsbDevType == CREATE_ETH_DESCRIPTOR))
			{
				...
				if(UsbEthWorkQueue)
				{
					...
					if(Req)
					{
						if(sizeof(Req->Message) >= sizeof(USBE_ETH_SEND_EOTF_MESSAGE))
						{
							memcpy(Req->Message, USBE_ETH_SEND_EOTF_MESSAGE, sizeof(USBE_ETH_SEND_EOTF_MESSAGE));
						}
						Req->Ep = ep;
						Req->DevNo = DevNo;
						if(sizeof(Req->Buffer) >= Len)
						{
		...
		else
		{
			...
			if( TDBG_IS_DBGFLAG_SET(usbe, DEBUG_DUMP_ETH_DATA) && (DevInfo[DevNo].UsbDevType == CREATE_ETH_DESCRIPTOR))
			{
				...
				if(UsbEthWorkQueue)
				{
					...
					if(Req)
					{
						if(sizeof(Req->Message) >= sizeof(USBE_ETH_SEND_FULP_MESSAGE))
						{
							memcpy(Req->Message, USBE_ETH_SEND_FULP_MESSAGE, sizeof(USBE_ETH_SEND_FULP_MESSAGE));
						}
						Req->Ep = ep;
						Req->DevNo = DevNo;
						if(sizeof(Req->Buffer) >= Len)
	...
	if (EpInfo->TxDataLen != EpInfo->TxDataPos)
	{
		...
		if( TDBG_IS_DBGFLAG_SET(usbe, DEBUG_DUMP_ETH_DATA) && (DevInfo[DevNo].UsbDevType == CREATE_ETH_DESCRIPTOR))
		{
			...
			if(UsbEthWorkQueue)
			{
				...
				if(Req)
				{
					if(sizeof(Req->Message) >= sizeof(USBE_ETH_SEND_EOTP_MESSAGE))
					{
						memcpy(Req->Message, USBE_ETH_SEND_EOTP_MESSAGE, sizeof(USBE_ETH_SEND_EOTP_MESSAGE));
					}
					Req->Ep = ep;
					Req->DevNo = DevNo;
					if(sizeof(Req->Buffer) >= Len)
	...
	{				
		...
		if( TDBG_IS_DBGFLAG_SET(usbe, DEBUG_DUMP_ETH_DATA) && (DevInfo[DevNo].UsbDevType == CREATE_ETH_DESCRIPTOR))
		{
			...
			if(UsbEthWorkQueue)
			{
				...
				if(Req)
				{
					if(sizeof(Req->Message) >= sizeof(USBE_ETH_SEND_ZERO_MESSAGE))
					{
						memcpy(Req->Message, USBE_ETH_SEND_ZERO_MESSAGE, sizeof(USBE_ETH_SEND_ZERO_MESSAGE));
					}
					Req->Ep = ep;
					Req->DevNo = DevNo;
					if(sizeof(Req->Buffer) >= Len)
...
void
UsbCoreDumpBotData(char *Title, uint8 ep, uint8 *Data, uint32 Len)
{
	int i;

	if (TDBG_IS_DBGFLAG_SET(usbe, DEBUG_DUMP_BOT_DATA))
	{
...
/* Ethernet over USB packet will be received inside the interrupt handler
 * callback only so dumping the packets directly there might causes kernel
 * to timeout/panic depending on the amount of packet we are dealing with
 * so to prevent that a copy of the packet will be posted to workqueue and
 * that copy will be available in the following workqueue callback
 * function which can be safely executed under the bottom half to dump
 * the packet(s) which are received/sent */
void
UsbCoreDumpEthData(struct work_struct *Work)
{
	USB_ETH_DATA_WORK *Req = NULL;
	struct timeval Tv = { 0 };
	uint32 Index = 0;

	Req = container_of(Work, USB_ETH_DATA_WORK, EthWork);
	if(Req)
	{
		jiffies_to_timeval(Req->Jiffies, &Tv);
		printk("%s (Time: %llu.%llu seconds.microseconds, EP:%d, DevNo:%d and Len:%d bytes)\n", Req->Message, (uint64)Tv.tv_sec, (uint64)Tv.tv_usec, Req->Ep, Req->DevNo, Req->Len);
		printk(KERN_CONT " -> ");
		for(; Index < Req->Len; Index++)
		{
			printk(KERN_CONT "0x%02X ", Req->Buffer[Index]);
		}
		printk(KERN_CONT " <- \n");
		kfree(Req);
	}
