...
FCGI_FILE *FCGI_fopen(const char *path, const char *mode)
{
    ...
    FCGI_FILE * fcgi_file = FCGI_OpenFromFILE(file);	/* Unreleased Resource: False Positive */
...
{
    FILE * file = tmpfile(); /* Fority[Unreleased resource] :: False Positive */
...
{
    FILE * file = fdopen(fd, mode); /* Fority[Unreleased resource] :: False Positive */
    FCGI_FILE * fcgi_file = FCGI_OpenFromFILE(file);
...
{
    FILE * file = popen(cmd, type); /* Fority[Unreleased resource] :: False Positive */
    FCGI_FILE * fcgi_file = FCGI_OpenFromFILE(file);
