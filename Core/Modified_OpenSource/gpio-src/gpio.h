...
#define IOCTL_GPIO_INT_REGISTER        _IOC(_IOC_WRITE,'K',0x21,0x3FFF)  // option for REGISTER GPIO for Interrupt
#define IOCTL_GPIO_INT_GET_STATUS      _IOC(_IOC_WRITE,'K',0x22,0x3FFF)  // option for Get Status of GPIO Interrupt
#define IOCTL_GPIO_INT_CLEAR           _IOC(_IOC_WRITE,'K',0x23,0x3FFF)  // option for Clear GPIO Interrupt 
#define IOCTL_GPIO_INT_UNREGISTER      _IOC(_IOC_WRITE,'K',0x24,0x3FFF)  // option for UnRegister GPIO Interrupt
#define IOCTL_GPIO_INT_WAIT_FOR        _IOC(_IOC_WRITE,'K',0x25,0x3FFF) // option for Wait for GPIO Interrupt
...
extern gpio_interrupt_sensor* intr_sensors;
extern uint32_t total_interrupt_sensors;
extern gpio_interrupt_sensor* intr_chassis;
extern uint32_t total_chassis_interrupts;
extern unsigned char g_gpiopinlist[256];

int process_gpio_intr_ext (int gpio_num, unsigned char state, unsigned char trigger_type);
void wakeup_intr_queue_ext (int intr_type);
