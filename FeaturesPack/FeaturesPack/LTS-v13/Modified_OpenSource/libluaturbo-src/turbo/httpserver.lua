...
-- callback given as argument on initialization. The callback recieved the
...
-- @param request_callback (Function) Callback when requests are recieve by
-- the server.
    ...
    if self.kwargs.read_body ~= false then
        ...
        if content_length then
            ...
            if type(self.kwargs.streaming_multipart_bytes) == "number" and
                ...
                local final_callback = function(self)
                    stream_parse.check_complete(stream_parse)
...
function httpserver.HTTPConnection:_on_request_body(data)
    ...
    if content_type then
        ...
        elseif content_type:find("multipart/form-data", 1, true) then
            ...
            -- some error like out of memory will cause parse_multipart_data() runtime error and will not write anything in response.
            -- use pcall to make sure parse_multipart_data() to run completely.
            local success, object = pcall(httputil.parse_multipart_data, self._request.body, boundary)
            if success then
                self.arguments = object or {}
            else
                -- send the key and value to notify redfish handler function to write the correct status code in response
                self.arguments = {
                    error_parse_multipart_data = {
                        message = object,
                        type = "others"
                    }
                }
