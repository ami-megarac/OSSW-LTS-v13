...
function iostream.IOStream:initialize(fd, io_loop, max_buffer_size, args)
    ...
    self.args = args or {}
    self._read_buffer = buffer(1024, true)
    self._read_buffer_size = 0
    self._read_buffer_offset = 0
    self._read_scan_offset = 0
    self._write_buffer = buffer(1024, true)
...
function iostream.IOStream:close()
    if self.socket then
        ...
        self._write_buffer:clear("BufferClearFlag")
        self._read_buffer:clear("BufferClearFlag")
    end
end
...
function iostream.IOStream:_read_from_buffer()
    ...
    if self._streaming_callback ~= nil and self._read_buffer_size ~= 0 then
        ...
        local success = false
        -- behavior like _run_callback_protected() but run directly
        local function __run_callback_protected(res)
            if self._streaming_callback_arg then
                return xpcall(self._streaming_callback, _run_callback_error_handler,
                    self._streaming_callback_arg, res)
            else
                return xpcall(self._streaming_callback, _run_callback_error_handler, res)
            end
        end
        if self._read_bytes ~= nil then
            bytes_to_consume = min(self._read_bytes, bytes_to_consume)
            self._read_bytes = self._read_bytes - bytes_to_consume
            success = __run_callback_protected(self:_consume(bytes_to_consume))
        else
            success = __run_callback_protected(self:_consume(bytes_to_consume))
        end
